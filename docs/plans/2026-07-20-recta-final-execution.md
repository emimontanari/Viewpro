...
|---|---|---|---|---|
| P0 | 🟡 | Aislamiento multi-tenant — backstop faseado (Opción B: Prisma `$extends` + ALS, elegido sobre RLS por el pooler de Neon). **Fase 1 hecha**: propagación de `tenantId` vía `nestjs-cls` (contexto ALS por request), sin tocar queries. | Fase 1: `#249` (`28ffa34`). Diseño completo en la sección de abajo del ledger / PR #249 | **Fase 2** (extension Prisma warn-mode) → **Fase 3** (enforce + tests cross-tenant) → **Fase 4** (guardrail estático + relacional). → Etapa 1. |
