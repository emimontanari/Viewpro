...
|---|---|---|---|---|
| P0 | 🟡 | Aislamiento multi-tenant — backstop faseado (Opción B: Prisma `$extends` + ALS). **Fase 1** (propagación ALS) + **Fase 2** (extension warn-mode) hechas. Warn mode midió el trabajo del enforce: **94 warnings en ~13 combos** model.op (dominante `TenantMembership.findUnique` 45×). | Fase 1 `#249` (`28ffa34`) · Fase 2 `#250` (`252e164`) | **Fase 3** (enforce: domar los 94 warnings + tests cross-tenant) → **Fase 4** (guardrail estático + relacional clase B). → Etapa 1. |
