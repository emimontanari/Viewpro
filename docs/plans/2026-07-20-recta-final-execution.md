...
|---|---|---|---|---|
| P0 | 🟢 | Aislamiento multi-tenant — backstop faseado (Opción B). **Fases 1-2-3a-3b hechas**: TODO el vector de LECTURA cerrado (findMany/count/bulk inyectan `where.tenantId`; findUnique/findUniqueOrThrow validan post-fetch → null/throw cross-tenant, hasta con `select` restrictivo). Probado con 2 tenants. Warnings 94→20. | `#249`·`#250`·`#251`·Fase 3b `#252` (`e4cdcef`) | **Fase 3c** (enforce writes by-id update/delete/upsert vía ownership check; 20 warnings restantes, menor riesgo) → **Fase 4** (guardrail estático + relacional clase B). → Etapa 1. |
