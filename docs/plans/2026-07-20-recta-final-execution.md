...
|---|---|---|---|---|
| P0 | ✅ | Aislamiento multi-tenant — backstop **enforzado (toda op clase A) + guardrail anti-regresión**. Fases 1→4. Cross-tenant read/write bloqueado (isolation spec 10/10); un modelo nuevo con `tenantId` sin registrar **rompe el CI** (registry vs schema). Warnings 94→0. | `#249`·`#250`·`#251`·`#252`·`#253`·Fase 4 `#254` (`7d890ea`) | **CERRADO.** Único follow-up opcional: aislamiento relacional clase B (`Document`/`PropertyAsset` por relación) — repos ya correctos, defensa-en-profundidad. |
