...
|---|---|---|---|---|
| P0 | 🟢 | Aislamiento multi-tenant — backstop faseado (Opción B). **Fase 1** (ALS) + **Fase 2** (warn) + **Fase 3a** (ENFORCE lecturas/bulk: inyecta `where.tenantId`; cross-tenant read **bloqueado**, probado con 2 tenants). El vector de fuga principal (leer datos ajenos) está cerrado. | `#249` · `#250` · Fase 3a `#251` (`e14f4ba`) | **Fase 3b** (enforce `findUnique`/writes by-id vía post-fetch validation; 75 warnings restantes) → **Fase 4** (guardrail estático + relacional clase B). → Etapa 1. |
