...
function dense(value, length) {
  if (!Array.isArray(value) || Object.getPrototypeOf(value) !== Array.prototype || value.length !== length || Reflect.ownKeys(value).length !== length + 1) return false;
...
function closed(value, expected, seen) {
  ...
  if (!dense(value.final, prefix.length + identities.length) || !dense(value.closure, value.closure.length)) return false;
  ...
  for (let i = 0; i < value.closure.length; i++) if (!closed(value.closure[i], identities, seen)) return false;
  return true;
}
