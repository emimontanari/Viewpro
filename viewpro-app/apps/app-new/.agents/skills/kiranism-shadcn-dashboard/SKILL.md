...
description: |
  Guide for building features, pages, tables, forms, themes, and navigation in this Next.js 16 shadcn dashboard template. Use this skill whenever the user wants to add a new page, create a feature module, build a data table, add a form, configure navigation items, add a theme, set up RBAC access control, or work with the dashboard's patterns and conventions. Also triggers when adding routes under /dashboard, working with ViewPro session/tenant membership UI, creating mock APIs, or modifying the sidebar. Even if the user doesn't mention "dashboard" explicitly — if they're adding UI, pages, or features to this project, use this skill.
---
...
Configure in `src/config/nav-config.ts`. Items are filtered client-side in `src/hooks/use-nav.ts` using the active ViewPro tenant membership from `SessionProvider`.

**Access control properties** on nav items:

- `requireOrg: boolean` — requires an active tenant membership
- `permission: string` — requires a specific ViewPro membership permission
- `role: string` — requires a specific ViewPro membership role

Items without `access` are visible to everyone. Navigation filtering is UX only; API/BFF guards remain the security source of truth.
