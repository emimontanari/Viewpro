...
## ViewPro baseline notes

This directory is a baseline snapshot of `Kiranism/next-shadcn-dashboard-starter` imported into the ViewPro monorepo as `apps/app-new`.

- Keep the starter's visual layout, styling, and components intact while the baseline is established.
- Wire ViewPro-specific backend/auth behavior in later small commits instead of mixing it into the import.
- Run locally from the monorepo root with `pnpm --filter next-shadcn-dashboard-starter dev`.
- Clerk auth, Clerk Billing, and Sentry are upstream template integrations in this baseline; replace or connect them to ViewPro in dedicated follow-up slices.
- Set `NEXT_PUBLIC_SENTRY_DISABLED="true"` to disable Sentry. Source map upload also requires Sentry org/project configuration.
