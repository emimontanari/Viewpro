...
export function assertSafeTestDatabaseUrl(
  ...
  if (!databaseUrl) {
    throw new Error(
      'Refusing to run platform-api tests without DATABASE_URL. Use a dedicated test database, e.g. viewpro_platform_test.',
    )
  }
  ...
  try {
    ...
    throw new Error('Refusing to run platform-api tests with an invalid DATABASE_URL.', {
      cause: error,
    })
  }
  ...
  if (DANGEROUS_DATABASE_NAMES.has(databaseName) || !isClearlyTestScoped) {
    throw new Error(
      [
        'Refusing to run platform-api tests against a non-test database.',
        `DATABASE_URL database: ${databaseName || '<missing>'}`,
        `DATABASE_URL schema: ${schemaName || '<default>'}`,
        'Use a dedicated database such as postgresql://viewpro_platform:viewpro_platform@localhost:5434/viewpro_platform_test.',
      ].join(' '),
    )
  }
}
