 ...
 *
 * Migrations are applied once to the template database and each worker database
 * is cloned from it with CREATE DATABASE ... TEMPLATE, which copies files rather
 * than replaying every migration. Cloning N databases therefore costs roughly
 * one migration run, not N.
 */
