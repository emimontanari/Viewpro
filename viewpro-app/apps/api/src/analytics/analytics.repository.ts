...
export type DashboardTopPropertyRecord = {
	engagementId: string;
	propertyId: string;
	title: string | null;
...
export type DashboardTopSellerRecord = {
	userId: string;
	name: string;
	email: string;
	movementCount: number;
...
export type AnalyticsRepository = {
	...
	countActiveEngagementsWithOwnerVisibleUpdate(
		input: TenantWindowInput,
	): Promise<number>;
	listActiveEngagementsWithoutRecentUpdate(
		input: TenantWindowInput,
