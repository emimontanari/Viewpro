...
const assignedAgentTenant: TenantContext = {
  ...
  permissions: [PERMISSIONS.ENGAGEMENTS_VIEW_ASSIGNED, PERMISSIONS.MOVEMENTS_CREATE],
}
...
describe('Movement use cases', () => {
  it('allows a manager to create a movement after verifying engagement visibility', async () => {
    ...
    expect(propertyEngagementsRepository.findByIdForTenant).toHaveBeenCalledWith({
      tenantId: 'tenant-1',
  ...
  it('rejects movement creation when the movement permission is missing', async () => {
    ...
    await expect(
      useCase.execute(
        ...
        currentUser,
        'engagement-1',
        { type: MovementType.GENERAL_UPDATE, observation: 'Missing permission.' },
      ),
    ).rejects.toThrow(new ForbiddenException('Insufficient permissions'))
    expect(propertyEngagementsRepository.findByIdForTenant).not.toHaveBeenCalled()
    expect(movementsRepository.create).not.toHaveBeenCalled()
  })
  ...
  it('maps paginated movement lists after verifying engagement visibility', async () => {
    ...
    const propertyEngagementsRepository = { findByIdForTenant: vi.fn().mockResolvedValue(engagement) }
    const movementsRepository = { findMany: vi.fn().mockResolvedValue({ items: [movement, secondMovement], total: 2 }) }
    const useCase = new ListMovementsUseCase(movementsRepository as never, propertyEngagementsRepository as never)
  ...
  it('rejects movement lists when neither engagement view permission is present', async () => {
    ...
    await expect(
      useCase.execute({ ...managerTenant, permissions: [PERMISSIONS.MOVEMENTS_CREATE] }, currentUser, 'engagement-1', {}),
    ).rejects.toThrow(new ForbiddenException('Insufficient permissions'))
    expect(propertyEngagementsRepository.findByIdForTenant).not.toHaveBeenCalled()
