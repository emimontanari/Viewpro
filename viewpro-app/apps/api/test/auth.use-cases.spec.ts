...
describe('Auth use cases', () => {
  it('emits seller login analytics when login resolves to exactly one tenant membership', async () => {
    ...
    const useCase = makeLoginUseCase({ analyticsService, memberships: [singleMembership] })

    const result = await useCase.execute({ email: 'Agent@Example.com', password: 'correct-password' })
  ...
  it('skips seller login analytics when multiple tenant memberships make tenant context ambiguous', async () => {
    ...
    const useCase = makeLoginUseCase({
      analyticsService,
      memberships: [singleMembership, { ...singleMembership, id: 'membership-2', tenantId: 'tenant-2', tenant: { ...singleMembership.tenant, id: 'tenant-2' } }],
    })

    await expect(useCase.execute({ email: 'agent@example.com', password: 'correct-password' })).resolves.toMatchObject({
      body: { user: { id: 'user-1' } },
    })
...
function makeLoginUseCase(input: { analyticsService: { track: ReturnType<typeof vi.fn> }; memberships: unknown[] }) {
  return new LoginUseCase(
    { findByEmail: vi.fn().mockResolvedValue(activeUser) } as never,
    { findManyByUserId: vi.fn().mockResolvedValue(input.memberships) } as never,
