...
describe('Owner portal (e2e)', () => {
  ...
  it('lists active owned properties without requiring x-tenant-id or leaking owner data', async () => {
    const manager = await registerTenantSession('owner-list-manager@example.com', 'Owner List Homes')
    const owner = await registerOwnerSession('owner-list@example.com', 'Owner List Temporary Homes')
    ...
    await createEngagement(manager.agent, manager.tenantId, { title: 'Unassigned Owner House' }).expect(201)
  ...
  it('returns property detail only to the active owner and hides inaccessible resources as 404', async () => {
    ...
    const otherOwner = await registerOwnerSession('other-owner-detail@example.com', 'Other Owner Detail Temporary Homes')
    const owned = await createEngagement(manager.agent, manager.tenantId, { title: 'Owned Detail House' }).expect(201)
    const revoked = await createEngagement(manager.agent, manager.tenantId, { title: 'Revoked Detail House' }).expect(201)
    await grantOwnerAccess(owner.userId, owned.body.property.id)
    await grantOwnerAccess(owner.userId, revoked.body.property.id, PropertyAssetOwnerAccessStatus.REVOKED)

    const visible = await owner.agent.get(`/api/owner/properties/${owned.body.property.id}`).expect(200)
    const otherOwnerResponse = await otherOwner.agent.get(`/api/owner/properties/${owned.body.property.id}`).expect(404)
    const revokedResponse = await owner.agent.get(`/api/owner/properties/${revoked.body.property.id}`).expect(404)

    expect(visible.body).toMatchObject({ id: owned.body.property.id, title: 'Owned Detail House' })
  ...
  it('lists owner property engagements with sanitized tenant and agent fields', async () => {
    const manager = await registerTenantSession('owner-engagements-manager@example.com', 'Owner Engagements Homes')
    const agent = await registerTenantSession('owner-engagements-agent@example.com', 'Owner Engagements Agent Homes')
    const owner = await registerOwnerSession('owner-engagements@example.com', 'Owner Engagements Temporary Homes')
    ...
    const response = await owner.agent.get(`/api/owner/properties/${engagement.body.property.id}/engagements`).expect(200)
    ...
    expect(response.body).toEqual([
      expect.objectContaining({
        ...
        publishedPriceCents: 150_000_00,
        currency: 'USD',
        agents: [{ userId: agent.userId, firstName: 'Owner', email: 'owner-engagements-agent@example.com' }],
      }),
    ])
  ...
  it('returns the owner timeline for owned engagements and 404 for non-owned engagements', async () => {
    const manager = await registerTenantSession('owner-timeline-manager@example.com', 'Owner Timeline Homes')
    const owner = await registerOwnerSession('owner-timeline@example.com', 'Owner Timeline Temporary Homes')
    const owned = await createEngagement(manager.agent, manager.tenantId, { title: 'Owner Timeline Property' }).expect(201)
    const hidden = await createEngagement(manager.agent, manager.tenantId, { title: 'Hidden Timeline Property' }).expect(201)
    await grantOwnerAccess(owner.userId, owned.body.property.id)
    const movement = await createMovement(manager.agent, manager.tenantId, owned.body.id, {
    ...
    const visible = await owner.agent
      .get(`/api/owner/engagements/${owned.body.id}/timeline?page=1&pageSize=10&order=asc`)
      .expect(200)
    const hiddenResponse = await owner.agent.get(`/api/owner/engagements/${hidden.body.id}/timeline`).expect(404)

    expect(visible.body).toMatchObject({ total: 1, page: 1, pageSize: 10 })
    expect(visible.body.engagement).toMatchObject({ id: owned.body.id, tenant: { id: manager.tenantId } })
    expect(visible.body.items).toEqual([
      expect.objectContaining({
        ...
        visitCount: 2,
        interestLevel: 'HIGH',
        createdBy: { id: manager.userId, email: 'owner-timeline-manager@example.com', firstName: 'Owner' },
      }),
    ])
  ...
  it('rejects unauthenticated owner requests', async () => {
    const response = await request(app.getHttpServer()).get('/api/owner/properties').expect(401)
