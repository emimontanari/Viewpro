...
describe("Property engagements (e2e)", () => {
  ...
  it("creates a manual invitation link for an invited owner without exposing token hashes", async () => {
    const manager = await registerTenantSession(
      "manager-owner-manual-link@example.com",
      "Manager Owner Manual Link Homes",
    );
    const created = await createEngagement(
      manager.agent,
      manager.tenantId,
    ...
    expect(response.body.invitationId).toEqual(expect.any(String));
    expect(response.body.expiresAt).toEqual(expect.any(String));
    expect(response.body.invitationUrl).toMatch(
      /^http:\/\/localhost:3000\/owner-invitations\//,
    );
    expect(response.body).not.toHaveProperty("tokenHash");
    expect(JSON.stringify(response.body)).not.toContain("tokenHash");

    const token = extractInvitationToken(response.body.invitationUrl);
    const invitationMetadata = await request(app.getHttpServer())
      .get(`/api/owner-invitations/${token}`)
      .expect(200);

    expect(invitationMetadata.body).toMatchObject({
  ...
  it("revokes older pending owner invitation links when rotating", async () => {
    const manager = await registerTenantSession(
      "manager-owner-rotate-link@example.com",
      "Manager Owner Rotate Link Homes",
    );
    const created = await createEngagement(
      manager.agent,
      manager.tenantId,
    ...
    const linkedOwner = await manager.agent
      ...
      .send({
        firstName: "Rotate",
        lastName: "Owner",
        email: "rotate-owner@example.com",
      })
      .expect(201);

    const firstLink = await manager.agent
      .post(
        `/api/property-engagements/${created.body.id}/owners/${linkedOwner.body.id}/invitation-link`,
      )
      .set("x-tenant-id", manager.tenantId)
      .expect(201);
    const secondLink = await manager.agent
      .post(
        `/api/property-engagements/${created.body.id}/owners/${linkedOwner.body.id}/invitation-link`,
      )
      .set("x-tenant-id", manager.tenantId)
    ...
    const firstToken = extractInvitationToken(firstLink.body.invitationUrl);
    const secondToken = extractInvitationToken(secondLink.body.invitationUrl);
    expect(firstToken).not.toBe(secondToken);

    const revokedResponse = await request(app.getHttpServer())
      .get(`/api/owner-invitations/${firstToken}`)
      .expect(410);
    ...
    await request(app.getHttpServer())
      .get(`/api/owner-invitations/${secondToken}`)
      .expect(200);
  ...
  it("returns conflict when generating a manual invitation link for an active owner", async () => {
    const manager = await registerTenantSession(
      ...
      "Manager Owner Active Link Homes",
    );
    await registerTenantSession(
      "active-owner-link@example.com",
      "Active Owner Link Homes",
    );
    const created = await createEngagement(
      manager.agent,
      manager.tenantId,
    ...
    const linkedOwner = await manager.agent
      ...
      .send({
        firstName: "Active",
        lastName: "Owner",
        email: "active-owner-link@example.com",
      })
      .expect(201);
  ...
  it("does not create manual invitation links for unrelated owner links", async () => {
    const manager = await registerTenantSession(
      "manager-owner-unrelated-link@example.com",
      "Manager Owner Unrelated Link Homes",
    );
    const firstEngagement = await createEngagement(
      manager.agent,
      manager.tenantId,
    ...
    const secondOwner = await manager.agent
      ...
      .send({
        firstName: "Other",
        lastName: "Owner",
        email: "other-owner-link@example.com",
      })
      .expect(201);

    const response = await manager.agent
      .post(
        `/api/property-engagements/${firstEngagement.body.id}/owners/${secondOwner.body.id}/invitation-link`,
      )
      .set("x-tenant-id", manager.tenantId)
      .expect(404);
  ...
  it("does not create manual invitation links for another tenant engagement", async () => {
    const tenantA = await registerTenantSession(
      "manager-owner-link-tenant-a@example.com",
      "Manager Owner Link Tenant A Homes",
    );
    const tenantB = await registerTenantSession(
      "manager-owner-link-tenant-b@example.com",
      "Manager Owner Link Tenant B Homes",
    );
    const createdByTenantB = await createEngagement(
      tenantB.agent,
      tenantB.tenantId,
    ).expect(201);
    const tenantBOwner = await tenantB.agent
      .post(`/api/property-engagements/${createdByTenantB.body.id}/owners`)
      .set("x-tenant-id", tenantB.tenantId)
      .send({
        firstName: "Tenant",
        lastName: "Owner",
        email: "tenant-b-owner-link@example.com",
      })
      .expect(201);

    const response = await tenantA.agent
      .post(
        `/api/property-engagements/${createdByTenantB.body.id}/owners/${tenantBOwner.body.id}/invitation-link`,
      )
      .set("x-tenant-id", tenantA.tenantId)
      .expect(404);
