...
describe('Documents repository foundation', () => {
  ...
  it('creates a document request with requester and owner ownership fields', async () => {
    const createdRequest = { id: 'request-1', requestedByUserId: 'agent-1', ownerUserId: 'owner-1' }
    const create = vi.fn().mockResolvedValue(createdRequest)
    const repository = new PrismaDocumentsRepository({ documentRequest: { create } } as never)
  ...
  it('lists manager-visible requests across the tenant without requester filtering', async () => {
    const items = [{ id: 'request-1', tenantId: 'tenant-1', requestedByUserId: 'agent-1' }]
    const findMany = vi.fn().mockResolvedValue(items)
    const count = vi.fn().mockResolvedValue(1)
    const repository = new PrismaDocumentsRepository({ documentRequest: { findMany, count } } as never)
  ...
  it('lists only requesting-seller-visible requests for non-manager viewers', async () => {
    const findMany = vi.fn().mockResolvedValue([{ id: 'request-1', requestedByUserId: 'agent-1' }])
    const count = vi.fn().mockResolvedValue(1)
    const repository = new PrismaDocumentsRepository({ documentRequest: { findMany, count } } as never)
  ...
  it('hides peer seller requests in internal detail queries', async () => {
    ...
    const repository = new PrismaDocumentsRepository({ documentRequest: { findFirst } } as never)
    ...
    expect(findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'request-1', tenantId: 'tenant-1', requestedByUserId: 'agent-2' },
      }),
    )
  })

  it('creates a pending document version with storage metadata', async () => {
    const createdDocument = { id: 'document-1', documentRequestId: 'request-1' }
    const createdVersion = { id: 'version-1', storageKey: 'documents/request-1/version-1.pdf' }
    ...
    const repository = new PrismaDocumentsRepository({ $transaction: transaction } as never)
