...
describe('PrismaTeamInvitationsRepository', () => {
  it('creates a pending invitation and stores only the token hash', async () => {
    const createdInvitation = pendingInvitation({ tokenHash: 'new-token-hash' })
    const tx = {
    ...
    const repository = new PrismaTeamInvitationsRepository({ $transaction: transaction } as never)
    ...
    expect(tx.teamInvitation.create.mock.calls[0][0].data).not.toHaveProperty('token')
  })

  it('returns alreadyMember for an existing same-tenant membership', async () => {
    const tx = {
      user: { findUnique: vi.fn().mockResolvedValue({ id: 'user-1' }) },
      tenantMembership: { findUnique: vi.fn().mockResolvedValue({ id: 'membership-1' }) },
      teamInvitation: { updateMany: vi.fn(), create: vi.fn() },
    }
    const repository = new PrismaTeamInvitationsRepository({ $transaction: vi.fn((callback) => callback(tx)) } as never)
  ...
  it('resends by revoking the previous pending invite and creating a fresh one', async () => {
    const tx = {
      teamInvitation: {
        ...
        updateMany: vi.fn().mockResolvedValue({ count: 1 }),
        create: vi.fn().mockResolvedValue(pendingInvitation({ id: 'invitation-2', tokenHash: 'fresh-hash' })),
      },
    }
    const repository = new PrismaTeamInvitationsRepository({ $transaction: vi.fn((callback) => callback(tx)) } as never)
    ...
    expect(result).toEqual({
      status: 'created',
      invitation: expect.objectContaining({ id: 'invitation-2', token: expect.any(String) }),
    })
    expect(tx.teamInvitation.findFirst).toHaveBeenCalledWith({ where: { id: 'invitation-1', tenantId: 'tenant-1' } })
    expect(tx.teamInvitation.updateMany).toHaveBeenCalledWith({
      where: { id: 'invitation-1', tenantId: 'tenant-1', status: TeamInvitationStatus.PENDING },
      data: { status: TeamInvitationStatus.REVOKED, revokedAt: now },
    })
    expect(tx.teamInvitation.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
  ...
  it('returns notFound when resending an invitation outside the tenant', async () => {
    const tx = { teamInvitation: { findFirst: vi.fn().mockResolvedValue(null) } }
    const repository = new PrismaTeamInvitationsRepository({ $transaction: vi.fn((callback) => callback(tx)) } as never)
  ...
  it('returns notAvailable when resending a non-pending or expired invitation', async () => {
    const tx = {
      teamInvitation: {
        findFirst: vi.fn().mockResolvedValue(pendingInvitation({ expiresAt: new Date('2026-05-30T10:00:00.000Z') })),
      },
    }
    const repository = new PrismaTeamInvitationsRepository({ $transaction: vi.fn((callback) => callback(tx)) } as never)
  ...
  it('revokes a pending invitation without returning a raw token', async () => {
    const revoked = pendingInvitation({ status: TeamInvitationStatus.REVOKED, revokedAt: now })
    const tx = {
      teamInvitation: {
        findFirst: vi.fn().mockResolvedValue(pendingInvitation()),
    ...
    const result = await repository.revokeInvitation({ tenantId: 'tenant-1', invitationId: 'invitation-1', now })

    expect(result).toEqual({ status: 'revoked', invitation: revoked })
    expect(result.status === 'revoked' ? result.invitation : {}).not.toHaveProperty('token')
  ...
  it('returns notFound when revoking an invitation outside the tenant', async () => {
    ...
    await expect(repository.revokeInvitation({ tenantId: 'tenant-2', invitationId: 'invitation-1', now })).resolves.toEqual({
      status: 'notFound',
    })
  })
})
