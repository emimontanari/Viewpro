...
describe("Analytics pilot reports", () => {
	it("computes active engagement update percentage for the current week by default", async () => {
		const repository = {
			...
			countActiveEngagementsWithOwnerVisibleUpdate: vi
				.fn()
				.mockResolvedValue(3),
		...
		expect(
			repository.countActiveEngagementsWithOwnerVisibleUpdate,
	...
	it("excludes active engagements with recent movement events from inactive reports", async () => {
		const analyticsEventFindMany = vi
			.fn()
			.mockResolvedValue([{ propertyEngagementId: "engagement-active" }]);
		...
		const inactive = await repository.listActiveEngagementsWithoutRecentUpdate({
			tenantId: "tenant-1",
			from,
			to,
		});
	...
	it("uses a seven-day default window for inactive engagements", async () => {
		...
		expect(
			repository.listActiveEngagementsWithoutRecentUpdate,
		).toHaveBeenCalledWith({
			tenantId: "tenant-1",
