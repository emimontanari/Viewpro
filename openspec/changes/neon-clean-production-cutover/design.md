...
| Delivery | `sequential-to-develop`: each WU starts from live `origin/develop`, then review/CI/merge to `develop`, never production `main`; remove → fetch/audit. Before EVERY WU audit `develop`, branches/worktrees/paths; overlap/new commit ⇒ refresh/re-plan. WU3 package/lock and WU7 `app-new`/session likely conflict. |
