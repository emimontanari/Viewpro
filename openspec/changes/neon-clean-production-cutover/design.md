...
|---|---|
| Candidate | Prefix: `main@868dc70` + #331/#333/#334/#335/#336. #347 produces reviewed WU2 candidate/null merge. After squash merge, closure PR from new `develop` binds actual SHA, marks WU2 complete, and gates WU3-WU7; no pre-merge digest alternative. WU3 commits tooling+versioned schema/template, never instance; WU3-WU7 emit reviewed merge receipts. Post-WU7 review/CI/merge, read-only tooling may assemble isolated candidate from prefix+WU1-WU7 patches in disposable worktree, compute deterministic tree/runtime/image digests, and checkpoint `release-manifest.v1.json` outside candidate Git/private evidence; independently reassemble identities/verify digests. Closure+reproduction plus single-use authorization permit promotion; public evidence/instance never alter bound tree. Pin tools/IDs, detached base, verified IDs, ordered cherry-picks, exact-tree CI/audit; reject hidden/optional dependencies/#314. |
| Delivery | `sequential-to-develop`: each WU starts from live `origin/develop`, reviews/CI/merges to `develop`, never `main`; remove→fetch/audit. Before each WU audit `develop`, branches/worktrees/paths; overlap/new commit ⇒ refresh/re-plan. WU3 package/lock and WU7 `app-new`/session likely conflict. |
...
| 2 | `viewpro-app/apps/viewpro-api/src/observability/**`, fixture specs, `viewpro-app/scripts/production-cutover/remediation-manifest.v1.json`: sanitized telemetry/alert/remediation; #347 produces a candidate receipt, then a post-merge closure PR binds its actual develop SHA and closes WU2. Revert. | 340 |
...
Forecast>400; `sequential-to-develop`/auto-chain, no size exception/strategy mixing; tasks retain the 390 hard stop and no provisioning authority.
