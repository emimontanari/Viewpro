...
## WU6 Backup Lineage — Task 3.2

- Task 3.2 is `[x]`; 9/18 tasks are complete. This five-path candidate adds the backup lineage validator and widens one prune cutoff. RED-CUT-08 was recovered from `800d1a3` and restated in the delta spec, because `e083fc3` removed the design table. Throughout, a prefix is an object-store key prefix, never the Git commit prefix the Lineage and Release contracts bind.
| TDD Cycle Evidence | Safety | RED | GREEN | TRIANGULATE | REFACTOR |
|---|---|---|---|---|---|
| 3.2 | 154/154 | module absent, then every hostile case failing | 28/28 | 62 defect mutations, all reachable ones caught | 28/28 |
- Focused `node --test scripts/production-cutover/backup-lineage.spec.mjs`: 28/28 passed, and again under four non-UTC zones; composed contract suites 154/154. One independent four-lens review ran before settlement and returned five blockers; one consolidated correction addressed them and a targeted validator re-ran every reviewer mutation under both UTC and a non-UTC zone.
- Design consequences of that review: collision is judged from position zero rather than on whole path segments, because an object store lists by byte prefix and the segment rule permitted exactly the `<name>` versus `<name>-<generation>` sweep its own rationale named as the threat; a calendar month overflows rather than clamps, because clamping 31 January to 28 February yields a twenty-eight day window, shorter than the thirty-day rule this replaces, and a retention boundary is a floor; retention compares epochs rather than ISO strings, because an expanded-year form sorts before every digit and read as unretained; `collides` answers "colliding" for a value it cannot compare; prune ownership is exact identity, never containment; and the whole prune-plan envelope is now validated by code tests actually execute.
- Declared, not claimed as covered: both fault reasons are unreachable while every guard refuses non-data before serialization, and the finiteness half of the retention comparison is unreachable while the reader validates each instant — the latter is retained because its absence fails open. A local-time implementation of the retention arithmetic is indistinguishable from a UTC one on a UTC runner, so the suite now runs a second time under `TZ=Pacific/Kiritimati`, which is what makes the anchoring observable where CI executes.
- `db-backup.yml` was changed by one token, from a thirty-day to a forty-five-day cutoff, which only ever retains more. The job remains per-object age rather than lineage-aware, so it still cannot honour a window that opens at the cutover; wiring the validator to the deleter, and moving uploads to generation-specific prefixes, are deliberately left to a later work unit because both change where production's only off-Neon copy lives. Budget reconciled from the historical `300–340` to the delivered `782/850`; the `<=400` review-size guidance remains waived for this change.

## WU7 Session and Rollback — Task 4.1

- Task 4.1 is `[x]`; 10/18 tasks are complete. This six-path candidate adds the session-invalidation suites for both lanes and the post-write reversal refusal. RED-CUT-12/13 were recovered from `800d1a3` and restated in the delta spec, because `e083fc3` removed the design table. RED-CUT-12's recovered row names the already-merged `checkpoint.mjs`; the refusal was placed in a new `rollback.mjs`, declared in the delta spec as an additive path rather than reopening a reviewed module.
| TDD Cycle Evidence | Safety | RED | GREEN | TRIANGULATE | REFACTOR |
|---|---|---|---|---|---|
| 4.1 | 177/177 | modules absent, then every hostile case failing | 23/23 + 6/6 + 10/10 | 26 defect mutations, all reachable ones caught | 23/23 + 6/6 + 10/10 |
- Focused `node --test scripts/production-cutover/rollback.spec.mjs`: 23/23; product session suite 6/6; platform session suite 10/10; composed contract suites 177/177. One independent four-lens review ran before settlement and returned four blocker classes; one consolidated correction addressed them and a targeted validator re-ran every reviewer mutation, including seven against the deployed guards and token services.
- The decisive finding was that the session suites proved nothing about production. Both were rebuilt onto the deployed `TokenService` and guards over a keyed configuration, so a generation now signs with its own real service and is judged by the next generation's real guard. Before the correction, eighteen of eighteen mutations to `auth.guard.ts`, `step-up.guard.ts` and `token.service.ts` survived — including replacing signature verification with `decode()`, a complete authentication bypass. After it, all seven re-run mutations are caught, among them that bypass, a step-up secret conflated with the access secret, a weakened `sessionExp` finiteness check, a removed absolute deadline, and reject paths that stopped clearing cookies.
