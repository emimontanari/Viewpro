...
The corrected strict units mechanically group into **26** dependency-ordered options. Execute C1→C2A→C2B1→C2B2→C3A→C3B→C4→C5A→C5B1→C5B2→C6A→C6B→C7…C20 and execute units left-to-right within each group; C5A/U5B must merge before C5B1/U6 and C5B1 must merge before C5B2/U6 races. C2B1 contains only the new S39 hardening spec, while C2B2 owns the reusable cleanup helper/direct matrix and retained C2A migration-smoke teardown retrofit. C2B2 is mandatory before C3A, C3B is mandatory before C4/U4B, C2B1/C2B2 are each ≤635, and every other displayed maximum is ≤650:
