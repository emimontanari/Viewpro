...
| U2A | `pnpm --filter @viewpro/api db:validate`; `pnpm --filter @viewpro/api exec vitest run test/property-proposal-schema.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Schema only; no leftover state |
| U2B | `pnpm --filter @viewpro/api db:validate`; `pnpm --filter @viewpro/api exec vitest run test/property-proposal-migration.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Migration/FK/index-lock evidence |
| U2C | `pnpm --filter @viewpro/api exec vitest run src/database/tenant-isolation.registry.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Registry and cleanup helper |
