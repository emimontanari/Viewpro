...
| C3A (U3) | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/domain/normalization.spec.ts src/property-proposals/domain/state-machine.spec.ts src/property-proposals/domain/replay-identity.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Pure domain only |
| C3B (U4A) | `pnpm --filter @viewpro/api exec vitest run src/property-engagements/active-property-engagement-capacity.spec.ts test/property-engagements.e2e-spec.ts`; `pnpm --filter @viewpro/api typecheck` | Capacity and direct manager path |
...
| C5A / U5B | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/create-property-proposal.use-case.spec.ts src/property-proposals/prisma-property-proposals.repository.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Seller draft creation, transactional eligibility, proposal identity, and duplicate title/address allowance |
| C5B1 / U6 (after C5A merges) | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/update-property-proposal.use-case.spec.ts src/property-proposals/prisma-property-proposals.repository.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Update, replay, scoped locks, and safe outcomes |
| C5B2 / U6 (after C5B1 merges) | `database_url='postgresql://viewpro:viewpro@127.0.0.1:5432/viewpro_test?schema=public'; DATABASE_URL="$database_url" DIRECT_URL="$database_url" pnpm --filter @viewpro/api exec vitest run --retry=0 test/property-proposal-eligibility-race.spec.ts` | Guarded real-PostgreSQL eligibility races and barrier cleanup |
| C6A | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/submit-property-proposal.use-case.spec.ts src/property-proposals/prisma-property-proposals.repository.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Initial BORRADOR submission and immutable round-one snapshot |
| C6B | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/submit-property-proposal.replay.spec.ts src/property-proposals/prisma-property-proposals.repository.spec.ts`; `pnpm exec turbo run typecheck --filter=@viewpro/api --force` | Rejected resubmit, retained history, and exact replay; API typecheck requires generated contract dependencies |
| C7A1 | `pnpm --filter @viewpro/api exec vitest run --retry=0 src/property-proposals/review-filter-builder.spec.ts`; `pnpm exec turbo run typecheck --filter=@viewpro/api --force`; `pnpm --filter @viewpro/api lint`; repeat the focused command; guarded local `_test` `pnpm --filter @viewpro/api exec vitest run --retry=0` | Pure S15 filter/pagination RED contract only; no repository consumption or reviewer-order evidence |
| C7A2 | `pnpm --filter @viewpro/api exec vitest run --retry=0 src/property-proposals/prisma-property-proposals.repository.spec.ts`; `pnpm exec turbo run typecheck --filter=@viewpro/api --force` | Pending S15 repository GREEN, including exact `COALESCE(latestSubmittedAt, createdAt) DESC, id DESC` ordering evidence |
| C7B | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/list-property-proposal-review.use-case.spec.ts src/property-proposals/use-cases/get-property-proposal-review.use-case.spec.ts`; `pnpm --filter @viewpro/api typecheck` | S11/S14 reviewer use-case behavior |
...
| U11B1 | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/approve-property-proposal.replay.spec.ts src/property-proposals/use-cases/approve-property-proposal.use-case.spec.ts src/property-proposals/use-cases/approve-property-proposal.quota.spec.ts`; `pnpm exec turbo run typecheck --filter=@viewpro/api --force`; `pnpm --filter @viewpro/api lint` | Replay ordering/source invariant; run focused specs twice |
| U11B2 | `pnpm --filter @viewpro/api exec vitest run --retry=0 test/property-proposal-approval-race.spec.ts` | Bounded same-proposal approval/approval and approval/rejection lock proof |
| U11B3 | `pnpm --filter @viewpro/api exec vitest run --retry=0 test/property-proposal-approval-race.spec.ts` | Bounded final-slot race proof |
...
| U15A1 | `pnpm --filter next-shadcn-dashboard-starter exec vitest run src/lib/bff-api.test.ts src/app/api/property-proposals/route.test.ts`; `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint` | Shared BFF helper and seller collection only |
| U15A2 | `pnpm --filter next-shadcn-dashboard-starter exec vitest run 'src/app/api/property-proposals/[proposalId]/route.test.ts' 'src/app/api/property-proposals/[proposalId]/submit/route.test.ts'`; `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint` | Seller detail and submit routes only |
| U15B1 reviewer reads | `pnpm --filter next-shadcn-dashboard-starter exec vitest run 'src/app/api/property-proposals/review/route.test.ts' 'src/app/api/property-proposals/review/[proposalId]/route.test.ts'` (run twice); `pnpm --filter @viewpro/contracts build`; `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint` | Collection/detail GET only: raw query bytes, one-time detail encoding, shared passthrough/error helpers, and no route-local tenant/auth behavior |
| U15B2 reviewer decisions | `pnpm --filter next-shadcn-dashboard-starter exec vitest run 'src/app/api/property-proposals/review/[proposalId]/reject/route.test.ts' 'src/app/api/property-proposals/review/[proposalId]/approve/route.test.ts'`; `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint` | Reject/approve only after U15B1; each route has its colocated test |
| U16A1 | `pnpm --filter next-shadcn-dashboard-starter exec vitest run src/lib/__tests__/bff-client.spec.ts` (run twice); `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint:strict` | BFF error boundary only |
| U16A2 | `pnpm --filter next-shadcn-dashboard-starter exec vitest run src/features/property-proposals/api/service.test.ts`; `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint:strict` | Typed proposal service after U16A1 |
| U16B1 query reads | `pnpm --filter next-shadcn-dashboard-starter exec vitest run src/features/property-proposals/api/queries.test.ts` (run twice); `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint:strict` | Tenant/audience keys, normalized read options/signals, and cancel-before-remove old-tenant cleanup only |
| U16B2 mutation cache | `pnpm --filter next-shadcn-dashboard-starter exec vitest run src/features/property-proposals/api/queries.test.ts`; `pnpm --filter next-shadcn-dashboard-starter typecheck`; `pnpm --filter next-shadcn-dashboard-starter lint:strict` | Five mutations, invalidation, 409 refresh, canonical product cache, and no optimistic state after U16B1 |
