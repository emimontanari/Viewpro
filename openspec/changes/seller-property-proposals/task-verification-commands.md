...
| C2A (U2A + U2B core + U2C registry) | `pnpm exec turbo run typecheck --filter=@viewpro/api --force`; `pnpm --filter @viewpro/api db:validate`; `pnpm --filter @viewpro/api exec vitest run test/property-proposal-schema.spec.ts test/property-proposal-migration.spec.ts src/database/tenant-isolation.registry.spec.ts` | Run the uncached compiler/Prisma generation before focused Prisma-importing Vitest; then prove readable schema, migrated-client source-link, and direct-tenant registry parity after the local guard. |
...
| C2B (mandatory before C3) | `pnpm exec turbo run typecheck --filter=@viewpro/api --force`; `pnpm --filter @viewpro/api exec vitest run test/property-proposal-migration.spec.ts src/database/tenant-isolation.registry.spec.ts` | Run the uncached compiler/Prisma generation before focused Prisma-importing Vitest; then cover deferred S39 decision/check, planner/index, deletion/update, duplicate, actual-DDL lock, and reusable cleanup hardening. |
