...
| C6B (after C6A merge) | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/submit-property-proposal.replay.spec.ts src/property-proposals/prisma-property-proposals.repository.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Rejected resubmit, retained history, and exact replay |
