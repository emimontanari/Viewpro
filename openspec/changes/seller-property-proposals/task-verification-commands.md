...
| C5B / U6 (after C5A merges) | `pnpm --filter @viewpro/api exec vitest run src/property-proposals/use-cases/update-property-proposal.use-case.spec.ts test/property-proposal-eligibility-race.spec.ts`; `pnpm --filter @viewpro/api typecheck` | Editable state and eligibility race |
