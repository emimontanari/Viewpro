...
- [ ] RED: From `viewpro-app/`, run `DOC` and `MODEL` against the landed D4a2 baseline, then add or complete only the public assertions in `property-document-requests.test.tsx` for rejection heading/reason and disabled review actions during a pending mutation; record passing characterization as baseline and do not manufacture a failing RED. <!-- sdd-owner: implementation -->
- [ ] GREEN: Move only `DocumentStatusConfig`, `documentStatusConfig`, `DocumentStatusBadge`, and `RejectionReason` into `request-list.tsx`; update the intact root item to import and use those exports without moving mutations, services, URL/query state, root imports, or state ownership. <!-- sdd-owner: implementation -->
...
- [ ] REFACTOR: Remove only the duplicated status/rejection primitive definitions, rerun `DOC` and `MODEL`, record the measured 225–280 artifact-inclusive diff and independent rollback boundary, and publish D4b1 before D4b2. <!-- sdd-owner: implementation -->
