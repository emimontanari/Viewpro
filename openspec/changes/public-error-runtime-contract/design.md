...
Compile one CommonJS contract, let native root Turbo order its two consumers, and prove API and App New in separate image processes. Vercel is a human blocking gate, not an automated toolchain.
...
| App New smoke | `src/instrumentation.ts` retains the static contract import. Only `NEXT_RUNTIME=nodejs` plus `VIEWPRO_RUNTIME_MARKER_PORT` dynamically imports `instrumentation-node.ts`; the helper alone owns `node:http`, port validation, awaited bind/rejection, then successful unref. This path is independent of Sentry. The standalone app and marker use different loopback ports. |
| Vercel release | A maintainer/operator attaches authenticated, deployment-specific evidence outside Git; a reviewer records pass/fail. No schema, capture, comparator, hash, alias, or Git tooling is introduced. |
...
| 3. App New marker/image smoke (390) | Modify only `scripts.runtime:smoke` in `viewpro-app/apps/app-new/package.json`, `viewpro-app/apps/app-new/Dockerfile`, and `viewpro-app/apps/app-new/src/instrumentation.ts`; create `viewpro-app/apps/app-new/src/instrumentation-node.ts`, `viewpro-app/apps/app-new/scripts/runtime-contract-image-smoke.mjs`, and `viewpro-app/apps/app-new/src/instrumentation.spec.ts`. Own static/dynamic-import gates, Node helper, marker env/host/method/body fields, separate ports, and bounded teardown. | Unit 1 → Node-only private marker and standalone smoke pass. RED evidence covers Node, absent env, Edge no-import, `EADDRINUSE`, and exact response. | Revert these App New paths; marker disappears with instrumentation change. |
| 4. CI/docs/manual gate (190) | Modify `viewpro-app/README.md`, `.github/workflows/ci.yml`; create `docs/release/manual-vercel-runtime-contract-gate.md`. Own sequential CI phases, root-command guidance, and checklist fields: deployment ID, full SHA, production, READY, settings, HTTPS URL, smoke, reviewer result. | Units 1–3 → checks documented and release gate usable. CI result plus authenticated evidence attached outside Git. | Revert only these docs/CI paths; never delete external evidence. |
