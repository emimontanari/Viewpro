...
| Vercel build/release | Only `viewpro-app/apps/app-new/vercel.json` is versioned, solely for the filtered-Turbo `buildCommand` that builds contracts before App New for both preview projects independently of dashboard settings. A maintainer/operator attaches authenticated evidence outside Git; all dashboard, deployment-policy, and manual-gate work remains Unit 4. |
