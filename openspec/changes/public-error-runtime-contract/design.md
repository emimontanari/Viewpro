...
| 3. App New marker/image smoke (390) | Modify only `scripts.runtime:smoke` in `viewpro-app/apps/app-new/package.json`, `viewpro-app/apps/app-new/{Dockerfile,next.config.ts,src/instrumentation.ts}`; create `viewpro-app/apps/app-new/src/instrumentation-node.ts`, `viewpro-app/apps/app-new/scripts/runtime-contract-image-smoke.mjs`, and `viewpro-app/apps/app-new/src/instrumentation.spec.ts`. Own static/dynamic-import gates, Node helper, marker env/host/method/body fields, separate ports, and bounded teardown. | Unit 1 → Node-only private marker and standalone smoke pass. RED evidence covers Node, absent env, Edge no-import, `EADDRINUSE`, and exact response. | Revert these App New paths; marker disappears with instrumentation change. |
