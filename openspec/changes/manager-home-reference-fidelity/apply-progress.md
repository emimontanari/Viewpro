...
**Structured status consumed:** authoritative OpenSpec `applyState: ready`, `apply: ready`, `nextRecommended: apply`; parent scoped the active second attempt `manager-home-i4-verification-evidence` to repo-local docs roots only, warning that all source/test surfaces are read-only.
**Candidate / provenance:** `feat/manager-home-reference-fidelity-i4`, base `d5a3ec2d0e4ef59616e38fc46ceed9fdd708a644`, settled code/test candidate tree `030425be780734f6a2afd59b9cb201c786647834`; subsequent corrections touch only `tasks.md`/`apply-progress.md` and leave code/tests byte-identical. Historical external-audit authority: correction settlement `revision2e`, evidence revision `sha256:454da4afd821dd09cba8dfdac274785bf0bc14aa8026e87aa90f5ec4ced5c517`.
**Completed persisted rows:** exactly I4 RED, GREEN, TRIANGULATE, and REFACTOR remain `[x]`; the frontend seam proves bounded permitted presenter records, populated/empty/error states, and valid/malformed/blank destinations; upstream API/BFF integration evidence owns archived/inactive/cross-tenant exclusions.
**Authority / deviation / boundary:** `DashboardSummaryResponse` includes descriptive status/tenant identifiers; inactive and tenant authorization stay upstream BFF/API-contract authority because presenters must not re-authorize or reinterpret already-authorized summary records. Docs-only correction; shared `lists.tsx` and seller block untouched; stacked-to-main I4 rollback removes only rankings.
### TDD Cycle Evidence
- **Safety** — `2026-09-07T12:34:10Z`: `pnpm --filter next-shadcn-dashboard-starter test src/features/dashboard/components/operational-homepage.test.tsx src/features/dashboard/components/operational-homepage/manager-home.test.tsx`; exit 0; 21 passed after `pnpm --filter @viewpro/contracts build`.
- **RED** — `2026-09-07T12:35:39Z`: `pnpm --filter next-shadcn-dashboard-starter test src/features/dashboard/components/operational-homepage.test.tsx src/features/dashboard/components/operational-homepage/manager-home.test.tsx`; exit 1; 2 intended missing-list/empty-state assertions.
- **GREEN** — `2026-09-07T12:40:15Z`: `pnpm --filter next-shadcn-dashboard-starter test src/features/dashboard/components/operational-homepage.test.tsx src/features/dashboard/components/operational-homepage/manager-home.test.tsx`; exit 0; 23 passed after minimum manager-specific regions.
- **TRIANGULATE** — `2026-09-07T12:40:54Z`: `pnpm --filter next-shadcn-dashboard-starter test src/features/dashboard/components/operational-homepage.test.tsx src/features/dashboard/components/operational-homepage/manager-home.test.tsx`; exit 0; 24 passed for bounded permitted presenter records, populated/empty/error, and valid/malformed/blank destinations; upstream API/BFF integration owns archived/inactive/cross-tenant exclusions.
- **REFACTOR** — `2026-09-07T12:42:54Z`: `pnpm --filter next-shadcn-dashboard-starter test src/features/dashboard/components/operational-homepage.test.tsx src/features/dashboard/components/operational-homepage/manager-home.test.tsx`; exit 0; 24 passed after fixture extraction.
- **Final focused** — `date -u` `2026-09-07T13:00:29Z`: `pnpm --filter next-shadcn-dashboard-starter test src/features/dashboard/components/operational-homepage.test.tsx src/features/dashboard/components/operational-homepage/manager-home.test.tsx`; exit 0; 2 files, 24 passed.
- **Final BFF** — `date -u` `2026-09-07T13:00:32Z`: `pnpm --filter next-shadcn-dashboard-starter test src/app/api/dashboard/summary/route.test.ts`; exit 0; 1 file, 5 passed.
- **Final owner** — `date -u` `2026-09-07T13:00:33Z`: `pnpm --filter next-shadcn-dashboard-starter test src/features/owner/components/owner-home.test.tsx`; exit 0; 1 file, 19 passed.
- **Final frontend** — `date -u` `2026-09-07T13:00:36Z`: `pnpm --filter next-shadcn-dashboard-starter test`; exit 0; 118 files, 776 passed.
- **Static / contract / accounting** — `date -u` `2026-09-07T13:01:00Z`: `pnpm --filter next-shadcn-dashboard-starter typecheck` exit 0 (`tsc --noEmit`); `2026-09-07T13:01:04Z`: `pnpm --filter next-shadcn-dashboard-starter lint:strict` exit 0 (`oxlint --deny-warnings`); `2026-09-07T13:03:37Z`: `env OPENSPEC_TELEMETRY=0 npx --yes @fission-ai/openspec validate manager-home-reference-fidelity --strict` exit 0 (`Change 'manager-home-reference-fidelity' is valid`); `2026-09-07T13:03:38Z`: `git -c diff.external= diff --check && git -c diff.external= diff --stat` exit 0; `2026-09-07T13:03:38Z`: `git -c diff.external= diff --numstat` exit 0, 345 additions + 55 deletions = 400; `git status --short` showed only the six candidate files.

**Scope / risks:** no browser, API, database, review, receipt, commit, push, PR, settle, or lifecycle operation ran; I5/I6 and verification rows remain unchanged and unchecked.
