...
- [x] GREEN only the I4 allowlist: render manager-only recent activity and top-properties lists from summary data with authorized engagement links and Argentina-local times; rerun focused components successfully. <!-- sdd-owner: implementation -->
- [x] TRIANGULATE I4 with bounded permitted presenter records, populated/empty/error states, and valid versus malformed/blank destinations; retain inactive/tenant authorization upstream because presenters must not re-authorize or reinterpret already-authorized summary records despite `DashboardSummaryResponse` descriptive status/tenant identifiers; rerun focused component and frontend suite commands successfully. <!-- sdd-owner: implementation -->
- [x] REFACTOR I4 without modifying shared seller list contracts, rerun focused/typecheck/lint, record evidence, measure ≤400 lines, and prepare its independent rollbackable PR. <!-- sdd-owner: implementation -->
...
| R5 — Activity and rankings use permitted source records | I4 bounded permitted presenter/destination proof plus upstream summary exclusion contract and I5 bounded-seller fixtures |
