...
Order: 1 → 2 → 3 → 4 → 5. WU3 and WU4 are independent of each other and may swap. Every unit is under the 400-line budget; WU2 is the only one close enough to watch. If its test matrix pushes it over, split at the module/wiring seam — WU2a the pure validator module and its tests (140–200), WU2b the DTO/use-case/repository wiring and its tests (90–140) — rather than absorbing the overflow.
