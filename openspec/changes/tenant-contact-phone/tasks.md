...
- [x] 5.3 REFACTOR: `pnpm --filter @viewpro/api exec vitest run && pnpm --filter @viewpro/api typecheck && pnpm --filter next-shadcn-dashboard-starter exec vitest run src/features/settings src/app/api/tenants && pnpm --filter next-shadcn-dashboard-starter typecheck`
