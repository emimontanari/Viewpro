...
- [ ] 2.7 In `use-cases/track-owner-movement-whatsapp-contact-click.use-case.ts`: change `targetType: 'movement_author'` literal to `'assigned_seller'` in the analytics metadata object (D5 — forward-only, no backfill). **Done-when**: no `'movement_author'` literal remains in `src/`.
- [ ] 2.8 Run `pnpm --filter @viewpro/api typecheck` — GREEN gate. **Done-when**: zero TypeScript errors.
